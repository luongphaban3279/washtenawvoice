<?php
/**
 * Template part for the grid display style
...
while ( $custom_query->have_posts() ) :
	...
	?>
	<article <?php post_class( 'newsplus entry-grid entry-port col'. $col . $fclass . $lclass . $ad_class ); echo $schema['container']; ?>>
		<?php
		if ( $ad_class ) {
		...
		?>
        <div class="port-overlay-wrap">
        
			<?php
    		printf( '<div class="port-overlay %s">', esc_attr( $overlay_style ) );
            echo $rows['row_1'];
