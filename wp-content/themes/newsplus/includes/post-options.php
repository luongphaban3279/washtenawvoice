...
$post_options = array(
		...
		'hide_meta'	 	=> array(
			'id' 			=> 'hide_meta',
			'title' 		=> __( 'Hide post meta on this post', 'newsplus' ),
			'type' 			=> 'checkbox'
		),
			...
			elseif ( $meta_box['type'] == 'select' ) { ?>
                <div>
                    ...
                    <select style="width:100%" name="<?php echo $meta_box['id']; ?>" id="<?php echo $meta_box['id']; ?>">
						<?php
                        foreach ( $meta_box['options'] as $index => $val ) {                        
							if ( is_numeric( $index ) && ( is_string( $val ) || is_numeric( $val ) ) ) {
								$option_label = $val;
								$option_value = $val;
							}
							
							elseif ( is_numeric( $index ) && is_array( $val ) ) {
								$option_label = isset( $val['label'] ) ? $val['label'] : array_pop( $val );
								$option_value = isset( $val['value'] ) ? $val['value'] : array_pop( $val );
							}
							...
							if ( isset( $data[ $meta_box['id'] ] ) && ( $data[ $meta_box['id'] ] == $option_value ) ) {
